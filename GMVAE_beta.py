...
class GMVAE(nn.Module):
	...
	def __init__(self, K=10, x_size=200, hidden_size=500, w_size=150):
		...
		# --------------encFcStack--------------
		# self.h1 = nn.Linear(self._after_bottlneck, self.hiddend_size, bias=False)
		# self.b4 = nn.BatchNorm1d(self.hiddend_size)
        # ------------------------------------------
		...
		# ----------priorStack-------------
		# self.h2 = nn.Linear(self.w_size, self.hiddend_size) # tanh activation
		# -----------------------------------------------------------------
		...
		# generative model
		# -----------------------decFcStack------------------------
		self.decFcStack_1 = nn.Sequential(
			nn.Linear(self.x_size, self.hiddend_size, bias=False),
			nn.BatchNorm1d(self.hiddend_size),
			nn.ReLU())

		self.decFcStack_2 = nn.Sequential(
		...
		# -----------------------decFcStack------------------------ 
		# self.h3 = nn.Linear(self.x_size, self.hiddend_size, bias=False)
		# self.b5 = nn.BatchNorm1d(self.hiddend_size)
		# self.h4 = nn.Linear(self.x_size, self._after_bottlneck, bias=False)
		# self.b6 = nn.BatchNorm1d(self._after_bottlneck)
		# -----------------------------------------------------------
		# ----------------------deconvStack------------------------ 
	...
	def decoder(self, x_sample):
		...
		h = self.decFcStack_1(x_sample)
		h = self.decFcStack_2(x_sample)
